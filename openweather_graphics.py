...
class OpenWeather_Graphics(displayio.Group):
    ...
    def display_weather(self, weather, localtime):
        # set the icon
        ...
        if not self.city_text:
            self.city_text = Label(self.small_font, text=city_name + " " + time_text)
            self.city_text.color = CITY_COLOR
            self._scrolling_texts.append(self.city_text)
        else:
            self.city_text.text = city_name + " " + time_text
    ...
    def display_black(self):
        splash = displayio.Group()
        background = displayio.OnDiskBitmap(open("blank.bmp", "rb"))
        bg_sprite = displayio.TileGrid(
            background,
            pixel_shader=getattr(background, 'pixel_shader', displayio.ColorConverter()),
        )
